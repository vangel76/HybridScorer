...
def launch_ui(initial_scores, image_paths, backend, script_dir, source_dir):
    ...
    def change_folder(new_path, pos_t, neg_t, pos_prompt, neg_prompt,
        ...
        if not new_path or not os.path.isdir(new_path):
            yield (gr.update(),) * 9 + (f"⚠️ Invalid folder: {new_path!r}",)
            return
        ...
        yield (gr.update(),) * 9 + (f"⏳ Scanning {new_path} …",)

        fresh_paths = scan_image_paths()
        if not fresh_paths:
            yield (gr.update(),) * 9 + (f"⚠️ No images found in {new_path}",)
            return
...
""")
        ...
        folder_out = [found_head, found_gallery,
                      notfound_head, notfound_gallery,
                      stats_box, pos_slider, neg_slider, hist_plot, sel_info, status]
        folder_btn.click(
            fn=change_folder,
