...
def launch_ui(initial_scores, image_paths, backend, script_dir, source_dir):
    ...
    def make_histogram(scores, pos_t, neg_t):
        """Return a matplotlib Figure with pos/neg score distributions + threshold lines."""
        ...
        n_plots  = 2 if has_neg else 1

        fig, axes = plt.subplots(n_plots, 1,
                                 figsize=(3.6, 2.6 * n_plots),
                                 facecolor="#0d0d11")
        if n_plots == 1:
            axes = [axes]

        def _draw(ax, vals, thresh, color, thresh_color, label):
            ax.set_facecolor("#0f0f16")
            for sp in ax.spines.values():
                sp.set_color("#2a2a38")
            ax.tick_params(colors="#667755", labelsize=7)
            ax.xaxis.label.set_color("#667755")
            ax.yaxis.label.set_color("#667755")

            if vals:
                n_bins = min(40, max(10, len(vals) // 3))
                ax.hist(vals, bins=n_bins, color=color,
                        edgecolor="#0d0d11", alpha=0.85)
            ax.axvline(thresh, color=thresh_color, linewidth=1.5,
                       linestyle="--", label=f"threshold {thresh:.3f}")
            ax.set_title(label, color="#aaccaa" if "pos" in label.lower() else "#ccaaaa",
                         fontsize=8, pad=3)
            ax.set_xlabel("score", fontsize=7)
            ax.set_ylabel("images", fontsize=7)
            ax.legend(fontsize=6, facecolor="#14141c", labelcolor="#aabb88",
                      edgecolor="#333344")
            ax.yaxis.set_major_locator(mticker.MaxNLocator(integer=True, nbins=4))

        _draw(axes[0], pos_vals, pos_t, "#336633", "#aadd66",
              f"positive  ({len(pos_vals)} images)")
        if has_neg:
            _draw(axes[1], neg_vals, neg_t, "#663333", "#dd6644",
                  f"negative  ({len(neg_vals)} images)")
