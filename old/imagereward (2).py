...
def calculate_imagereward_scores(image_paths, model, device):
    ...
    for i in tqdm(range(0, len(image_paths), chunk_size), desc="Scoring with ImageReward"):
...
def launch_ui(initial_scores, image_paths, model, device, script_dir, source_dir):
    ...
    def rescore(new_prompt, threshold, progress=gr.Progress()):
        ...
        if not new_prompt:
            yield (gr.update(),) * 6 + ("⚠️ Prompt cannot be empty.",)
            return

        yield (gr.update(),) * 6 + ("⏳ Scoring with new prompt…",)

        try:
            fresh_paths = scan_image_paths()
            state["image_paths"] = fresh_paths
            if not fresh_paths:
                yield (gr.update(),) * 6 + ("⚠️ No images found.",)
                return

            progress(0, desc=f"Scoring {len(fresh_paths)} images…")
            new_scores = {}
            for i in tqdm(range(0, len(fresh_paths), BATCH_SIZE), desc="Scoring"):
                batch_paths = fresh_paths[i:i + BATCH_SIZE]
                try:
                    with torch.no_grad():
                        _, batch_rewards = state["model"].inference_rank(new_prompt, batch_paths)
                        for path, reward in zip(batch_paths, batch_rewards):
                            fname = os.path.basename(path)
                            new_scores[fname] = {"score": float(reward), "path": path}
                except Exception as e:
                    for path in batch_paths:
                        fname = os.path.basename(path)
                        new_scores[fname] = {"score": -float('inf'), "path": path}
                if state["device"] == "cuda":
                    torch.cuda.empty_cache()
                progress(min((i + BATCH_SIZE) / len(fresh_paths), 1.0))

            state["scores"] = new_scores
            state["overrides"] = {}
        except Exception as e:
            yield (gr.update(),) * 6 + (f"❌ Error: {e}",)
            return

        lo, hi, mid = slider_range(new_scores)
        best, normal = build_split(new_scores, {}, mid)
        status = status_line(best, normal, new_scores, {})
    ...
    def change_folder(new_path, threshold, progress=gr.Progress()):
        ...
        if not new_path or not os.path.isdir(new_path):
            yield (gr.update(),) * 7 + (f"⚠️ Invalid folder: {new_path!r}",)
            return

        state["source_dir"] = new_path
        state["overrides"] = {}
        yield (gr.update(),) * 7 + (f"⏳ Scanning {new_path}…",)

        fresh_paths = scan_image_paths()
        if not fresh_paths:
            yield (gr.update(),) * 7 + (f"⚠️ No images in {new_path}",)
            return

        state["image_paths"] = fresh_paths
        progress(0, desc=f"Scoring {len(fresh_paths)} images…")
        new_scores = calculate_imagereward_scores(fresh_paths, state["model"], state["device"])

        # Wrap scores in the format we expect
        wrapped_scores = {}
        for fname, score_val in new_scores.items():
            if fname in [os.path.basename(p) for p in fresh_paths]:
                path = next(p for p in fresh_paths if os.path.basename(p) == fname)
                wrapped_scores[fname] = {"score": score_val, "path": path}

        state["scores"] = wrapped_scores
        progress(1)

        lo, hi, mid = slider_range(wrapped_scores)
        best, normal = build_split(wrapped_scores, {}, mid)
        status = status_line(best, normal, wrapped_scores, {})
    ...
    def on_best_select(sel: gr.SelectData, threshold):
        ...
        state["best_sel"] = sel.index
        state["normal_sel"] = None
        best, _ = build_split(state["scores"], state["overrides"], threshold)
        fname = os.path.basename(best[sel.index][0]) if sel.index < len(best) else "?"
    ...
    def move_to_best(threshold):
        ...
        idx = state.get("normal_sel")
        if idx is not None:
            _, normal = build_split(state["scores"], state["overrides"], threshold)
            if idx < len(normal):
                fname = os.path.basename(normal[idx][0])
...
""")
        ...
        # ── wiring ───────────────────────────────────────────────────────────
        all_out = [best_head, best_gallery, normal_head, normal_gallery, status, hist_plot]
        ...
        hist_click_out = [best_head, best_gallery, normal_head, normal_gallery, status, hist_plot, threshold_slider]
        hist_plot.select(
            fn=on_hist_click,
        ...
        rescore_out = [best_head, best_gallery, normal_head, normal_gallery,
                       status, hist_plot, threshold_slider]
        rescore_btn.click(
            fn=rescore,
