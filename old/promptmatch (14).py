...
def launch_ui(initial_scores, image_paths, backend, script_dir, source_dir):
    ...
    def make_histogram(scores, pos_t, neg_t):
        """Return a Plotly figure — click on a bar sets the threshold at that x."""
        ...
        fig = make_subplots(rows=n_rows, cols=1,
                            ...
                            subplot_titles=(
                                [f"▲ positive — click to set threshold  (now {pos_t:.3f})",
                                 f"▼ negative — click to set threshold  (now {neg_t:.3f})"]
                                if has_neg else
                                [f"▲ positive — click to set threshold  (now {pos_t:.3f})"]
                            ))

        def _add(row, vals, thresh, bar_color, line_color):
            if vals:
                fig.add_trace(go.Histogram(
                    x=vals,
                    nbinsx=min(40, max(10, len(vals) // 3)),
                    marker_color=bar_color,
                    marker_line_color="#0d0d11",
                    marker_line_width=0.5,
                    opacity=0.85,
                    showlegend=False,
                ), row=row, col=1)
            # Threshold vertical line as a shape
            fig.add_vline(
                x=thresh, line_color=line_color,
                line_dash="dash", line_width=2,
                row=row, col=1,   # type: ignore
            )

        _add(1, pos_vals, pos_t, "#3a7a3a", "#aadd66")
        if has_neg:
            _add(2, neg_vals, neg_t, "#7a3a3a", "#dd6644")
        ...
        fig.update_layout(
            height=h, margin=dict(l=34, r=8, t=36, b=28),
            paper_bgcolor=bg, plot_bgcolor=bg,
            font=dict(color=txt, size=9),
            clickmode="event",   # enables plotly_click
            dragmode=False,      # disable zoom/pan drag — clicks fire cleanly
        )
        fig.update_xaxes(gridcolor=grid, zerolinecolor=grid, tickfont_size=8)
        fig.update_yaxes(gridcolor=grid, zerolinecolor=grid, tickfont_size=8)
        # Subplot title colour
        for ann in fig.layout.annotations:
            ann.font.color = "#99bb88"
            ann.font.size  = 9
        return fig
    ...
    /* histogram plot */
    .hist-plot { margin-top:4px !important; border-radius:6px !important;
                 overflow:hidden !important; cursor:crosshair !important; }
    .hist-plot > .wrap { padding:0 !important; }
    /* hide plotly modebar (zoom/pan/save buttons) */
    #hist-plot .modebar { display:none !important; }
    """
...
<div class='subhead'>CLIP-powered interactive image sorter
 · click image to select, then use ← → buttons to move · ✋ = manual override
 · <span style='color:#aadd66'>click histogram to set threshold</span></div>
<script>
(function(){
  // Wait for Plotly div to appear, then attach click listener
  function attachPlotlyClick(){
    // Gradio wraps gr.Plot in a div; the inner plotly div has class "js-plotly-plot"
    const plotDiv = document.querySelector('#hist-plot .js-plotly-plot');
    if(!plotDiv){
      setTimeout(attachPlotlyClick, 500);
      return;
    }
    if(plotDiv._pmAttached) return;
    plotDiv._pmAttached = true;

    // Disable toolbar and re-apply no-drag config after each render
    if(window.Plotly){
      window.Plotly.relayout(plotDiv, {dragmode: false});
    }
    plotDiv.on('plotly_click', function(data){
      if(!data || !data.points || !data.points.length) return;
      const pt       = data.points[0];
      const x        = pt.x;
      // curveNumber 0 = positive subplot, 1 = negative subplot
      const side     = pt.curveNumber === 0 ? 'pos' : 'neg';
      const payload  = side + ':' + x.toFixed(4);

      // Write to hidden Gradio textbox and fire input event
      const box = document.querySelector('#hist-click-signal textarea');
      if(box){
        // Append tiny noise so Gradio detects a change even if same value
        box.value = payload + '|' + Date.now();
        box.dispatchEvent(new Event('input', {bubbles:true}));
      }
    });
    console.log('[PromptMatch] Plotly click listener attached');
  }
  // Re-attach whenever the plot rerenders (after rescore/reload)
  setInterval(attachPlotlyClick, 1000);
})();
</script>
""")
        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=1, min_width=290):
                ...
                hist_plot = gr.Plot(
                    ...
                    show_label=False,
                    elem_id="hist-plot",
                    elem_classes=["hist-plot"],
                )
                # Hidden textbox — JS writes "pos:0.243" or "neg:0.187" here
                # when user clicks the Plotly chart; triggers on_hist_click
                hist_click_signal = gr.Textbox(
                    value="", visible=False, elem_id="hist-click-signal")
