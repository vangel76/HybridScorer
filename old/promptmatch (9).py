...
def launch_ui(initial_scores, image_paths, backend, script_dir, source_dir):
    ...
    state = {
        ...
        "found_sel": None,
        "nf_sel":    None,
        "backend":   backend,   # current ModelBackend instance
    }
    ...
    def reload_model(model_label, pos_t, neg_t, pos_prompt, neg_prompt,
        ...
        if cfg is None:
            yield (gr.update(),)*8 + (f"⚠️ Unknown model: {model_label}",)
            return

        label, be, kwargs = cfg
        yield (gr.update(),)*8 + (f"⏳ Loading {label} …",)

        try:
            new_backend = ModelBackend(device, backend=be, **kwargs)
            state["backend"] = new_backend
        except Exception as e:
            yield (gr.update(),)*8 + (f"❌ Load error: {e}",)
            return
        ...
        try:
            ...
            new_scores = score_all(image_paths, new_backend, pos_emb, neg_emb,
                                   progress_cb=_cb)
            progress(1, desc="Done")
        except Exception as e:
            yield (gr.update(),)*8 + (f"❌ Scoring error: {e}",)
            return
    ...
    def rescore(new_pos, new_neg, pos_t, neg_t, progress=gr.Progress()):
        ...
        try:
            ...
            def _cb(done, total):
                progress(done / total, desc=f"Scoring… {done}/{total}")
            new_scores = score_all(image_paths, cur, pos_emb, neg_emb,
                                   progress_cb=_cb)
            progress(1, desc="Done")
        except Exception as e:
    ...
    with gr.Blocks(title="PromptMatch") as demo:
        ...
        reload_out = [found_head, found_gallery,
                      notfound_head, notfound_gallery,
                      stats_box, pos_slider, neg_slider, sel_info, status]
        reload_btn.click(
            fn=reload_model,
