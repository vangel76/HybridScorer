...
(function(){
  function initHist(){
    ...
    if(!wrap || wrap._pmInit){ setTimeout(initHist, 400); return; }
    wrap._pmInit = true;
    wrap.addEventListener('click', function(e){
      const geom = JSON.parse(wrap.dataset.geom);
      const rect = wrap.getBoundingClientRect();
      // normalise click to image pixel coords
      ...
      if(box){
        box.value = side + ':' + clamped.toFixed(4) + '|' + Date.now();
        box.dispatchEvent(new Event('input', {bubbles:true}));
      }
    });
  }
  // Re-init after every Gradio update (hist img gets replaced)
  new MutationObserver(function(){
    const wrap = document.getElementById('pm-hist-wrap');
    if(wrap) wrap._pmInit = false;
    initHist();
