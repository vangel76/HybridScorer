...
def get_windows_folder_placeholder():
    return r"C:\path\to\images"


def get_windows_allowed_paths(*extra_paths):
    allowed = []
...
def launch_ui(initial_scores, image_paths, model, device, script_dir, source_dir, server_port):
    try:
        import gradio as gr
    except ImportError:
...
if __name__ == "__main__":
    ...
    server_port = resolve_server_port(7860, "IMAGEREWARD_PORT")
    print(f"Launching browser UI on http://localhost:{server_port} …\n")
    launch_ui(scores, image_paths, model, device, script_dir, source_dir, server_port)
